...
import styles from './AppGroupList.less';
...
@connect(({ appGroupModel, loading, authCheckModel }) => ({
  appGroupModel,
  authCheckModel,
  loading: loading.models.appGroupModel,
}))
// @Form.create() 是一个注解，就简化了xxx = Form.create(xxx);export xxx
@Form.create()
class AppGroupList extends PureComponent {
  state = {
  ...
  getPageData(pageNo, searchParamInput) {
    ...
    const {
        appGroupModel: { searchParam, pager },
    } = this.props;
  ...
  setTableLoading = () => {
    const { dispatch } = this.props;
    dispatch({
      type: 'appGroupModel/setTableLoading',
    });
  };
  ...
  render() {
    const {
       appGroupModel: { selectState, groupAllCodeList },
    } = this.props;
    ...
    const {
       appGroupModel: { totalNumber, pager, tableList, tableLoading },
    } = this.props;
