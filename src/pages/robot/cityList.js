...
const CreateForm = Form.create()(prop => {
  ...
  return (
    ...
    >
      ...
      <FormItem labelCol={{ span: 6 }} wrapperCol={{ span: 16 }} label="状态：1新地址，0老地址" hasFeedback>
        {form.getFieldDecorator('status', {
          rules: [{ required: true, message: '请输入状态：1新地址，0老地址！' }],
        })(
          <Input placeholder="请输入状态：1新地址，0老地址" />)}
      </FormItem>
    </Modal>
  );
});
...
const EditForm = Form.create()(props => {
  ...
  return (
    ...
    >
      ...
      <FormItem labelCol={{ span: 6 }} wrapperCol={{ span: 16 }} label="状态：1新地址，0老地址">
        {form.getFieldDecorator('status', {
...
@connect(({ cityModel, loading, authCheckModel }) => ({
  cityModel,
  authCheckModel,
...
class CityList extends PureComponent {
  ...
  expandedRowRender = record => (
    <div>
      <Row>
        ...
        <Col span={6}>
          <Badge status="success" text="状态：1新地址，0老地址：" />
  ...
  setTableLoading = () => {
    const { dispatch } = this.props;
    dispatch({
      type: 'cityModel/setTableLoading',
    });
  };
  ...
  render() {
    const {
       cityModel: { selectState, groupAllCodeList },
    } = this.props;
    ...
    const {
       cityModel: { totalNumber, pager, tableList, tableLoading },
    } = this.props;
