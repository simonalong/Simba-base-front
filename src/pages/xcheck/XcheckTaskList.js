...
import styles from './XcheckTaskList.less';
...
@connect(({ authCheckModel, xcheckTaskModel, xcheckModel, xcheckCommonModel, loading }) => ({
  authCheckModel,
  xcheckTaskModel,
  xcheckCommonModel,
  xcheckModel,
  loading: loading.models.xcheckTaskModel,
}))
// @Form.create() 是一个注解，就简化了xxx = Form.create(xxx);export xxx
@Form.create()
class XcheckTaskList extends PureComponent {
  state = {
  ...
  componentDidMount() {
    ...
    const {
      xcheckTaskModel: { activePaneName },
    } = this.props;
    // console.log('启动');
  ...
  getPageDate(name, pageNo, searchParam) {
    ...
    if (index > -1) {
      ...
      // 获取页面的总个数
      dispatch({
        type: 'xcheckTaskModel/pageCount',
        payload: {
          paneIndex: index,
          searchParam: param,
        },
  ...
  expandedRowRender = record => (
    <div>
      <Row>
        <Col span={6}>
          <Badge status="success" text="模板变量值：" />
          <span>{record.tplVarJson}</span>
        </Col>
      </Row>
      <br />
    </div>
  ...
  setTableLoading = () => {
    const { dispatch } = this.props;
    dispatch({
      type: 'xcheckTaskModel/setTableLoading',
    });
  };
  ...
  getActivePaneIndex = () => {
    const {
      xcheckTaskModel: { activePaneName, panes },
    } = this.props;
  ...
  renderSearchForm = () => {
    ...
    const {
      ...
      xcheckCommonModel: { taskStatusDict },
      location : {xcheckId }
    } = this.props;
            ...
            <FormItem label="状态">
              {getFieldDecorator('status')(
                <Select
                  ...
                  optionFilterProp="children"
                >
                  {taskStatusDict.map(d => <Select.Option key={d.val}>{d.desc}</Select.Option>)}
                </Select>
              )}
            </FormItem>
          </Col>
  ...
  onEdit = (targetKey, action) => {
    const { dispatch } = this.props;
    const {
       xcheckTaskModel: { panes, maxTabIndex, activePaneName, tabIndexList },
    } = this.props;
    ...
    if (action === 'remove') {
      ...
      if (activePaneNameStr !== targetKey) {
        dispatch({
          type: 'xcheckTaskModel/deletePane',
          payload: {
        ...
        if (newActivePaneName !== '0') {
          dispatch({
            type: 'xcheckTaskModel/deletePaneActive',
            payload: {
              panes: panes.filter(pane => pane.name !== targetKey),
              tabIndexList: tabIndexList.filter(tableIndex => tableIndex !== targetKey),
  ...
  onTabChange = activePaneName => {
    ...
    dispatch({
      type: 'xcheckTaskModel/activePane',
  ...
  render() {
    const {
       xcheckTaskModel: { selectState, groupAllCodeList },
    } = this.props;
    ...
    const {
       xcheckTaskModel: { panes, activePaneName },
    } = this.props;
