...
import styles from './XcheckBillList.less';
...
const CreateForm = Form.create()(prop => {
  const { modalVisible, form, handleAdd, hideAddModal, xcheckBillTypeDict} = prop;
  const okHandle = () => {
  ...
  return (
    ...
    >
      <FormItem labelCol={{ span: 6 }} wrapperCol={{ span: 16 }} label="类型">
        {form.getFieldDecorator('type', {
          ...
          >
            {xcheckBillTypeDict.map(d => <Select.Option key={d.val}>{d.desc}</Select.Option>)}
          </Select>
        )}
      </FormItem>
      <FormItem labelCol={{ span: 6 }} wrapperCol={{ span: 16 }} label="账单名" hasFeedback>
        {form.getFieldDecorator('name', {
          rules: [{ required: true, message: '请输入账单名！' }],
        })(
          <Input placeholder="请输入账单名" />)}
      </FormItem>
      ...
      <FormItem labelCol={{ span: 6 }} wrapperCol={{ span: 16 }} label="加载配置(json)" hasFeedback>
        {form.getFieldDecorator('loadingConfigJson', {
          rules: [{ required: false, message: '' }],
        })(
          <Input placeholder="请输入加载配置(json)" />)}
      </FormItem>
      <FormItem labelCol={{ span: 6 }} wrapperCol={{ span: 16 }} label="账单列" hasFeedback>
        {form.getFieldDecorator('columnListJson', {
          rules: [{ required: true, message: '请输入账单列！' }],
        })(
          <Input placeholder="请输入账单列" />)}
      </FormItem>
    </Modal>
  );
});
...
const EditForm = Form.create()(props => {
  const { modalVisible, form, handleEdit, hideEditModal, item , xcheckBillTypeDict} = props;
  const okHandle = () => {
    form.validateFields((err, fieldsValue) => {
      if (err) {
        return;
  ...
  return (
    ...
    >
      <FormItem labelCol={{ span: 6 }} wrapperCol={{ span: 16 }} label="类型">
        {form.getFieldDecorator('type', {
          initialValue: item.type !== undefined ? item.type.toString() : '',
      ...
      <FormItem labelCol={{ span: 6 }} wrapperCol={{ span: 16 }} label="账单名">
        {form.getFieldDecorator('name', {
      ...
      <FormItem labelCol={{ span: 6 }} wrapperCol={{ span: 16 }} label="加载配置(json)">
        {form.getFieldDecorator('loadingConfigJson', {
          initialValue: item.loadingConfigJson,
          rules: [{ required: true, message: '请输入加载配置(json)！' }],
        })(
          <Input placeholder="请输入加载配置(json)" />
        )}
      </FormItem>
      <FormItem labelCol={{ span: 6 }} wrapperCol={{ span: 16 }} label="账单列">
        {form.getFieldDecorator('columnListJson', {
          initialValue: item.columnListJson,
          rules: [{ required: true, message: '请输入账单列！' }],
        })(
          <Input placeholder="请输入账单列"  />
        )}
...
@connect(({ authCheckModel, xcheckBillModel, xcheckCommonModel, loading }) => ({
  authCheckModel,
  xcheckBillModel,
  xcheckCommonModel,
  loading: loading.models.xcheckBillModel,
}))
// @Form.create() 是一个注解，就简化了xxx = Form.create(xxx);export xxx
@Form.create()
class XcheckBillList extends PureComponent {
  state = {
  ...
  componentDidMount() {
    ...
    const {
      xcheckBillModel: { activePaneName },
    } = this.props;
    // console.log('启动');
  ...
  getPageDate(name, pageNo, searchParam) {
    ...
    if (index > -1) {
      ...
      // 获取页面的总个数
      dispatch({
        type: 'xcheckBillModel/pageCount',
        payload: {
          paneIndex: index,
          searchParam: param,
        },
  ...
  expandedRowRender = record => (
    <div>
      <Row>
        <Col span={6}>
          <Badge status="success" text="加载配置(json)：" />
          <span>{record.loadingConfigJson}</span>
        </Col>
      </Row>
      <br />
    </div>
  ...
  setTableLoading = () => {
    const { dispatch } = this.props;
    dispatch({
      type: 'xcheckBillModel/setTableLoading',
    });
  };
  ...
  getActivePaneIndex = () => {
    const {
      xcheckBillModel: { activePaneName, panes },
    } = this.props;
  ...
  renderSearchForm = () => {
    ...
    const {
      xcheckCommonModel: { xcheckBillTypeDict },
    } = this.props;
  ...
  onEdit = (targetKey, action) => {
    const { dispatch } = this.props;
    const {
       xcheckBillModel: { panes, maxTabIndex, activePaneName, tabIndexList },
    } = this.props;
    ...
    if (action === 'remove') {
      ...
      if (activePaneNameStr !== targetKey) {
        dispatch({
          type: 'xcheckBillModel/deletePane',
          payload: {
        ...
        if (newActivePaneName !== '0') {
          dispatch({
            type: 'xcheckBillModel/deletePaneActive',
            payload: {
              panes: panes.filter(pane => pane.name !== targetKey),
              tabIndexList: tabIndexList.filter(tableIndex => tableIndex !== targetKey),
  ...
  onTabChange = activePaneName => {
    ...
    dispatch({
      type: 'xcheckBillModel/activePane',
  ...
  render() {
    const {
       xcheckBillModel: { selectState, groupAllCodeList},
    } = this.props;
    ...
    const {
       xcheckBillModel: { panes, activePaneName },
    } = this.props;
